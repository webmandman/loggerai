...
export function QueryResults({ answer, entries = [] }: QueryResultsProps) {
    ...
    <div className="relative overflow-hidden rounded-2xl border border-primary/10 bg-gradient-to-br from-card via-card to-primary/[0.02] shadow-sm animate-in fade-in slide-in-from-top-3 duration-500">
      ...
      {entries.length > 0 && (
        <div className="border-t border-primary/10">
          ...
          {showLogs && (
            <div className="pb-2 animate-in fade-in slide-in-from-top-2 duration-300">
              {entries.map((entry) => {
                const isExpanded = expandedLogId === entry.id;
                const date = new Date(entry.createdAt);
                const dateStr = date.toLocaleDateString([], { month: "short", day: "numeric" });
                return (
                  <div key={entry.id} className="border-t border-border/30 first:border-t-0">
                    <button
                    ...
                    >
                      <span className="shrink-0 text-[11px] text-muted-foreground/60 tabular-nums w-12">
                        {dateStr}
                      </span>
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-3 pt-1 pl-[4.25rem] animate-in fade-in duration-200">
