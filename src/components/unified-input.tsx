          ...
          for (const line of lines) {
            ...
            try {
              ...
              if (evt.type === "delta") {
                ...
                if (Array.isArray(evt.relevantEntryIds) && evt.relevantEntryIds.length > 0) {
                  try {
                    const batchRes = await api("/api/logs/batch", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ ids: evt.relevantEntryIds }),
                    });
                    if (batchRes.ok) {
